...
Resource                        PageObjects_EIL_InsuredInfo.robot
...
Verify already added Endorsements for EIL
    ...
    VerifyText                  EIL-1322-0821
    VerifyText                  Claims Notice Document
    Verifytext                  17                          anchor=EIL-1322-0821        partial_match=false

    VerifyText                  EIL-1029-0120
    VerifyText                  No Coverage for Electronic Data
    Verifytext                  18                          anchor=EIL-1029-0120        partial_match=false

    VerifyText                  EIL-1093-0422
    VerifyText                  Mold-Legionella Exclusion Endorsement
    Verifytext                  19                          anchor=EIL-1093-0422        partial_match=false

    VerifyText                  EIL-1411-0524
    VerifyText                  PFAS Exclusion Endorsement
    Verifytext                  20                          anchor=EIL-1411-0524        partial_match=false
...
Verify Default Endorsements for EIL
    ...
    #ClickElement               //lightning-icon[@title\='Endorsements']
    BuiltIn.Import Variables    /home/services/suite/PageObjects/EIL/endorsements_eil.py                          EIL_VA
    # Import Variables          ${CURDIR}/endorsements1.py                              EIL_VA
    Log To Console              ${CURDIR}
...
Verify Clearing house Fees  
    ...
    ${tria_premium}=            GetInputValue               //input[contains(@name,'-Clearing house Fee')]
    Log To Console              ${tria_premium}

    ${tria_premium_cleaned} =                               Evaluate                    "${tria_premium}".replace('$', '')
    Log To Console              ${tria_premium_cleaned}

    ${quote_premium}=           GetInputValue               //input[contains(@name,'-Primary Quote Premium')]
    Log To Console              ${quote_premium}

    ${quote_premium_cleaned} =                              Evaluate                    "${quote_premium}".replace('$', '').replace(',', '')
    Log To Console              ${quote_premium_cleaned}

    ${clearing_house_fee} =     Evaluate                    float(${quote_premium_cleaned}) * 0.00175
    Log To Console              ${clearing_house_fee}

    ${clearing_house_fee_rounded} =     Evaluate                    round(${clearing_house_fee}, 2)
    Log To Console              ${clearing_house_fee_rounded}

    ${clearing_house_fee_str}=        Evaluate    f"{${clearing_house_fee_rounded}:.2f}"
    Log To Console    ${clearing_house_fee_str}

    IF                          '${tria_premium_cleaned}' == '${clearing_house_fee_str}'
        Log To Console          Success Tria Premium is ${clearing_house_fee_str}
    ELSE
        Fail
    END
